...
def main():
    ...
    try:
        ...
        logging.error(f"관리자 로그인 테스트 중 오류가 발생 했습니다: {str(e)}")
        return
    try:
        challenge_tab(driver,wait)
        print("챌린지 탭 진입 성공")
    except Exception as e:
        logging.error(f"챌린지 탭 진입 테스트 중 오류가 발생 했습니다: {str(e)}")
        return
    try:
        challenge_register(driver, wait)
        print("챌린지 탭 등록 버튼 클릭 성공")
    except Exception as e:
        logging.error(f"챌린지 탭 등록 버튼 클릭 테스트 중 오류가 발생 했습니다: {str(e)}")
        return
    try:
        book_name_value = "해리" # 도서명 검색
        challenge_name_value = "서하 자동화 테스트 - 신청 직후 03" # 챌린지명 입력
        choice_book(driver, wait, challenge_name_value, book_name_value)
        print("도서 선택 & 챌린지 명 입력 성공")
    except Exception as e:
        logging.error(f"도서 선택 & 챌린지 명 입력 테스트 중 오류가 발생 했습니다: {str(e)}")
        return
    try:
        ...
        logging.error(f"챌린지 상세 입력 테스트 중 오류가 발생 했습니다: {str(e)}")
        return
    try:
        ...
        logging.error(f"모집 기간 설정 완료 테스트 중 오류가 발생 했습니다: {str(e)}")
        return
    try:
        ...
        logging.error(f"챌린지 기간 설정 테스트 중 오류가 발생 했습니다: {str(e)}")
        return
    try:
        ...
        logging.error(f"메이트 설정 완료 테스트 중 오류가 발생 했습니다: {str(e)}")
        return
    try:
    ...
    try:
        ...
        logging.error(f"문학동네 홈페이지 진입 테스트 중 오류가 발생 했습니다: {str(e)}")
        return
    try:
        munhak_login(driver, wait, user_id, user_pw)
        print("로그인 성공")
    except Exception as e:
        logging.error(f"로그인 테스트 중 오류가 발생 했습니다: {str(e)}")
        return
    try:
        ...
        logging.error(f"독파 진입 테스트 중 오류가 발생 했습니다: {str(e)}")
        return
    try:
