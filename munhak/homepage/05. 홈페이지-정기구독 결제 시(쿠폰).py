...
from munhak.module.homepage_common_functions import (
    home_page, munhak_login, munhak_mypage, quarterly_click, sub_date_set, sub_init_set, giveaway_book, payment_coupon_agree, payment_click, my_sub_list)
...
def main():
    ...
    sub_date_set(driver, wait, "period1_01")
    # 정기구독 개시 설정  // 봄 , 여름 , 가을 , 겨울
    sub_init_set(driver, "가을")
    # 증정도서 선택
    giveaway_book(driver)
    # 결제(쿠폰) 선택
    payment_coupon_agree(driver)
    # 결제하기 선택
