...
def challenge_apply_confirm(driver, wait):
    ...
    driver.execute_script("arguments[0].scrollIntoView(true);", required_agreement)  # 요소가 보이도록 스크롤
    click(driver, required_agreement)  # JavaScript로 클릭
    time.sleep(1)
    print("필수동의 체크 완료")
    # '참여하기' 버튼 확인
    challenge_apply_button = wait.until(EC.element_to_be_clickable((By.XPATH, '//*[@id="joinBtn"]/span')))
    driver.execute_script("arguments[0].scrollIntoView(true);", challenge_apply_button)  # 요소가 보이도록 스크롤
    click(driver,challenge_apply_button) # JavaScript로 클릭
    time.sleep(1)
    print("참여하기 선택 완료")
    # 챌린지 신청 완료 팝업
    ...
    driver.execute_script("arguments[0].scrollIntoView(true);", challenge_complete_popup)  # 요소가 보이도록 스크롤
    click(driver,challenge_complete_popup)  # JavaScript로 클릭
    print("확인 버튼 클릭 완료")
    time.sleep(1)
