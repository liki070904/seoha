...
from munhak.module.dokpa_common_functions import (
    home_page, munhak_login, dokpa_enter, some_function, all_challenge_apply, challenge_list, secret_challenge_list)
...
def main():
    ...
    try:
        ...
        return
    try:
        nondscsPwd = "1234" # 시크릿 비번 입력
        secret_radio(driver, wait, nondscsPwd)
        print("시크릿 라디오, 비번 입력 성공")
    except Exception as e:
        logging.error(f"시크릿 라디오, 비번 입력 테스트 중 오류가 발생했습니다: {str(e)}")
        return
    try:
        book_name_value = "해리" # 도서명 검색
        challenge_name_value = "서하 자동화 테스트 - 시크릿 챌린지 01" # 챌린지명 입력
    ...
    try:   #현재월 : 1 / 다음월 : 2 / 오늘(start_index) : 0 / 내일(end_index) : 1
        ...
        return
    try:
        mate_name_value = "서하" # 메이트명 겁색
        search_mate(driver, wait, mate_name_value)
        print("메이트 설정 완료")
    except Exception as e:
    ...
    try:
        ...
        return
    try:
        secret_challenge_popup_value = "1234" # 시크릿 비밀번호 입력
        time.sleep(1)
        secret_challenge_list(driver, wait, challenge_name_value, secret_challenge_popup_value)
        time.sleep(1)
        print("시크릿 챌린지 신청 버튼 클릭 성공")
    except Exception as e:
